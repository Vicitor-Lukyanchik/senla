...
public class ReservationDaoImpl implements ReservationDao {
    ...
    public void create(Reservation reservation, Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement(INSERT_RESERVATION, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void createWithId(Reservation reservation, Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement(INSERT_RESERVATION_WITH_ID, Statement.RETURN_GENERATED_KEYS)) {
    ...
    public void update(Reservation reservation, Connection connection) {
        try (PreparedStatement statement = connection.prepareStatement(UPDATE_RESERVATION, Statement.RETURN_GENERATED_KEYS)) {
            statement.setDate(1, Date.valueOf(reservation.getStartDate()));
            statement.setDate(2, Date.valueOf(reservation.getEndDate()));
            statement.setLong(3, reservation.getLodgerId());
            statement.setLong(4, reservation.getRoomId());
